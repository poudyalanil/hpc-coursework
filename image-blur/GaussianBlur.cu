...
// GPU Func
__global__ void blur(unsigned int height, unsigned int width){
...
int main(int argc, char **argv){
  ...
  const char* program is henewFileName = "filtered.png";
  ...
  blur<<<height, width>>>(height, width);

  cudaMemcpy(newImage, d_out, ARRAY_BYTES, cudaMemcpyDeviceToHost);
