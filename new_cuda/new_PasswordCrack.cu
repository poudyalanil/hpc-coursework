...
char hashed_password[] = "ZVYDWY3171";

__device__ char* CudaCrypt(char* rawPassword, char* result){
	...
	for(int i =0; i<10; i++){
		...
		// if(newPassword[i] == hashed_password) {
			// result = newPassword[i];
		// }
	}
	return (char *) newPassword[1];
}
...
char * goo = CudaCrypt(genRawPass, result);
printf("%s", (char)goo);
...
char * gpuResult;
char * cpuResult;
cudaMalloc( (void**) &gpuResult, sizeof(char) * 5); 
cudaMemcpy(gpuResult, cpuResult, sizeof(char) * 5, cudaMemcpyHostToDevice);



crack<<< dim3(26,26,1), dim3(10,10,1) >>>( gpuAlphabet, gpuNumbers, gpuResult );
