...
char* hashed_password = "sordwy5112";
char* bruteforced = CudaCrypt(genRawPass);
int iter = 0;
int flag = 0;
while (*hashed_password != '\0' || *bruteforced != '\0') {
	if (*hashed_password == *bruteforced) {
		hashed_password++;
		bruteforced++;
	} else if ((*hashed_password == '\0' && *bruteforced != '\0') || (*hashed_password != '\0' && *bruteforced == '\0') || *hashed_password != *bruteforced) {
		flag = 1;
		// printf("Not Equal\n");
		break;
	}
...
if (flag == 0) {
	printf("%c %c %c %c \n", genRawPass[0],genRawPass[1],genRawPass[2],genRawPass[3]);
	// printf("Equal \n");
}
